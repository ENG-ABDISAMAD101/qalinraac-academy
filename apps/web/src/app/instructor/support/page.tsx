...
import {
  ...
  instructorSupportDetailRequest,
  instructorSupportListRequest,
  uploadFileRequest,
  type InstructorSupportTicket,
} from "@/lib/api";
...
function statusTone(status: string) {
  ...
  return "bg-muted text-foreground dark:bg-[#1A1A1A]";
}
...
export default function InstructorSupportPage() {
  ...
  const loadTickets = useCallback(async () => {
    ...
    setListError("");
    try {
      setTickets(await instructorSupportListRequest());
    } catch (err) {
  ...
  async function expandTicket(id: string) {
    if (openId === id) {
      setOpenId(null);
      return;
    }
    ...
    try {
      const detail = await instructorSupportDetailRequest(id);
      setTickets((list) =>
        list.map((t) => (t.id === id ? { ...t, ...detail } : t)),
      );
    } catch {
      // List payload is enough for issue tab; discussions may stay empty.
    } finally {
                ...
                return (
                  ...
                  >
                    <button
                      ...
                      onClick={() => void expandTicket(t.id)}
                      aria-expanded={open}
                    >
                              ...
                              >
                                ...
                                <div>
                                  ...
                                  <select
                                    ...
                                    value={t.status}
                                    disabled
                                    className="flex h-11 w-full max-w-xs rounded-full border border-input bg-muted px-4 text-sm text-muted-foreground"
                                    title="Status is managed by support staff"
                                  >
                                  ...
                                  <p className="mt-1 text-xs text-muted-foreground">
                                    Status updates are handled by the support
                                    team.
                                  </p>
                                </div>
                              </TabsContent>
                              ...
                              >
                                {replies.length === 0 ? (
                                  <p className="rounded-xl border border-dashed border-border px-4 py-6 text-center text-sm text-muted-foreground">
                                    No messages yet. Replies from support will
                                    appear here.
                                  </p>
                                ) : (
                                  <ul className="space-y-3">
