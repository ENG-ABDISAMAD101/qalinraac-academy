...
const FILTERS: Array<"All" | ResearchProjectStatus> = [
  "All",
  "Draft",
  "Active",
  "Under Review",
...
export default function ResearchProjectsPage() {
  ...
  const rows = useMemo(() => {
    return demoResearchProjects.filter((p) => {
      if (filter !== "All" && p.status !== filter) return false;
      const hay = `${p.title} ${p.area} ${p.researchers.join(" ")}`.toLowerCase();
      ...
      <div className="space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Title, area, methodology, researchers, progress, and status
          </p>
        </div>
              ...
              <tr key={p.id} className="border-b border-border/70 hover:bg-accent/40">
                ...
                <StickyActionCell>
                  <Button asChild size="sm" variant="outline">
                    <Link href={`/research/projects/${p.id}`}>View</Link>
                  </Button>
                </StickyActionCell>
              </tr>
            ))}
