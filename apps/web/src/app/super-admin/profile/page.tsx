...
import { demoSuperAdmin } from "@/lib/super-admin-demo-data";

export default function SuperAdminProfilePage() {
  const { user } = useAuth();
  const [fullName, setFullName] = useState(
    user?.fullName ?? demoSuperAdmin.fullName,
  );
  const [phone, setPhone] = useState(demoSuperAdmin.phone);
  const [bio, setBio] = useState(demoSuperAdmin.bio);
      ...
      <div className="mx-auto max-w-3xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <form onSubmit={onSubmit} className="card-soft space-y-5 p-6">
          <div className="flex items-center gap-4">
            <Image
              src={user?.avatarUrl || demoSuperAdmin.avatarUrl}
              alt=""
              width={72}
              height={72}
          ...
          <div className="grid gap-3 rounded-2xl bg-muted p-4 text-sm sm:grid-cols-2 lg:grid-cols-4">
            <div>
              ...
              <p className="font-semibold">
                {user?.email ?? demoSuperAdmin.email}
              </p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-semibold">01 Jan 2025</p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground">Account status</p>
              <p className="font-semibold">{demoSuperAdmin.accountStatus}</p>
            </div>
          </div>
