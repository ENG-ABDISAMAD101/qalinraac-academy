...
import { demoFinance } from "@/lib/finance-demo-data";

export default function FinanceProfilePage() {
  const { user } = useAuth();
  const [fullName, setFullName] = useState(
    user?.fullName ?? demoFinance.fullName,
  );
  const [phone, setPhone] = useState(demoFinance.phone);
  const [bio, setBio] = useState(demoFinance.bio);
      ...
      <div className="mx-auto max-w-3xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <form onSubmit={onSubmit} className="card-soft space-y-5 p-6">
          <div className="flex items-center gap-4">
            <Image
              src={user?.avatarUrl || demoFinance.avatarUrl}
              alt=""
              width={72}
              height={72}
          ...
          <div className="grid gap-3 rounded-2xl bg-muted p-4 text-sm sm:grid-cols-3">
            ...
            <div>
              ...
              <p className="font-semibold">01 Sep 2025</p>
            </div>
          </div>
