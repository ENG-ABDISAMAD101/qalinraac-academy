...
export function AcademicProfileMenu() {
  const { user, logout } = useAuth();
  const router = useRouter();
  const name = user?.fullName ?? "Academic";
  const email = user?.email ?? "academic@qalinraac.local";
  const initials = name
    .split(" ")
    .map((p) => p[0])
    .join("")
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-64">
        ...
        <DropdownMenuItem asChild>
          <Link href="/academic/dashboard">
            <LayoutDashboard />
            Dashboard
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <Link href="/academic/profile">
            <UserRound />
            Profile
          </Link>
        </DropdownMenuItem>
        <DropdownMenuItem asChild>
          <Link href="/academic/notifications">
            <Bell />
            Notifications
          </Link>
        </DropdownMenuItem>
